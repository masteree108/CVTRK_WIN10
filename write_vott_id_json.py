...
# private
    ...
    def __create_asset_id_via_md5(self, path):
        ...
        h = m.hexdigest()
        self.pym.PY_LOG('D', self.__class__, 'via md5 to create asset_id: %s' % h)
...
# public
    def __init__(self, target_path):
        ...
        if os.path.exists(target_path):
            ...
            self.pym.PY_LOG('D', self.__class__, 'traget_path: %s is existed' % target_path)
        else:
            self.pym.PY_LOG('E', self.__class__, 'traget_path: %s is not existed!!' % target_path)
    ...
    def create_id_json_file(self, json_file_path):
        try:
            ...
            self.pym.PY_LOG('D', self.__class__, 'new json file_path: %s' % new_json_file_path)
            ...
            self.pym.PY_LOG('E', self.__class__, 'write VoTT id json file failed!!')
