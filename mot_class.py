...
# public    
    def __init__(self, frame, bboxes, algorithm):
        ...
        self.__processor_task_num  = self.__assign_amount_of_people_for_tracker(self.__detect_amount_of_people, self.__using_processor_qty)
        ct = 0;
        for i in range(self.__using_processor_qty):
            ...
            processes = multiprocessing.Process(
                        ...
                        args = (frame, bboxes_for_trackers, algorithm, iq, oq))
    ...
    def tracker_process(self, frame, bboxes, algorithm, inputQueue, outputQueue):
        #print("tracker_process")
        tracker = cv2.MultiTracker_create()
        for i,bbox in enumerate(bboxes):
            tracker.add(self.__get_algorithm_tracker(algorithm), frame, bbox)
