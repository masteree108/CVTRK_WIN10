...
# private
    ...
    def __judge_object_confidence_label_to_get_bbox_for_crop(self, layerOutputs, boxes, crop_width, crop_height, crop_x, crop_y, classIDs):
        #confidences = []
    ...
    def __judge_object_confidence_label_to_get_bbox_for_full(self, layerOutputs, boxes, W, H, classIDs):
        for output in layerOutputs:
            # loop over each of the detections
            for detection in output:
                # extract the class ID and confidence (i.e., probability)
...
# public
    ...
    def run_detection(self, frame, output_image_sw):
        ...
        for crop_x in range(0, 2840, 960):
            for crop_y in range(0, 1620, 540):
                ...
                self.__judge_object_confidence_label_to_get_bbox_for_crop(layerOutputs, boxes, crop_width, crop_height, crop_x, crop_y, classIDs)
        ...
        self.__net.setInput(blob)
        layerOutputs = self.__net.forward(ln)
        self.__judge_object_confidence_label_to_get_bbox_for_full(layerOutputs, boxes, W, H, classIDs)
